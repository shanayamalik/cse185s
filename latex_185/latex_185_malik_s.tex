...
Insert inline equations using dollar signs (\$). For example, the famous energy-mass equivalence formula by Einstein can be written inline like this:

\begin{verbatim}
The formula \( E = mc^2 \) is an example of an inline equation.
\end{verbatim}

The actual formula will look like this in your document: \( E = mc^2 \).

\subsubsection{Display Equations}
Display equations are separated from the text and typically centered. The `equation` environment is used for this purpose:
...
It will appear in your document like this:
...
\subsection{Symbols}
LaTeX supports an extensive array of mathematical symbols. Greek letters, summation symbols, and integral symbols can be used as follows:

\begin{verbatim}
The Greek letter delta: \( \delta \) or \( \Delta \)
The summation symbol: \( \sum \)
The integral symbol: \( \int \)
\end{verbatim}

\subsection{Fractions}
Fractions are created with the `\frac` command:

\begin{verbatim}
A simple fraction looks like this: \( \frac{a}{b} \).
\end{verbatim}

In your document, it will appear as: \( \frac{a}{b} \).
